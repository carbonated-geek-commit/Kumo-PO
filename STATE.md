...
Mobile (≤900px): the tray summary BAR is site-wide (shared partial `partials/tray-bar.njk`). On the menu page it lives in the sticky `.menu-stack` under the section nav, always visible (count + running total + View). On every other page a `--global` copy sticks directly under the header, appears only when the tray has items, and adds a "Menu" shortcut. The old floating pill is gone. Inside the drawer, "← Keep browsing the menu" navigates to /menu/ — except on the menu page, where it just closes the drawer. The section nav is scrollspy-driven: desktop chips get `.is-active` on the current section; mobile condenses to [★ Favorites home] [current section] [next section →] (current hides while in Favorites to avoid the duplicate). The spy runs directly on scroll/resize events — deliberately NOT rAF-deferred, since rAF (and rAF-aligned scroll events) don't fire in backgrounded tabs.
